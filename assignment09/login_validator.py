...
with open('users_500.csv', 'r') as file:
...
while True:
    print("Press the return key to end the program.")
    username = input('Please enter a username to search for: ')
    if username == '':
        break
    password = input('Please enter a password to search for: ')
    if (UserInfo(username, password)) in tree:
        print('valid')
    else:
        print('not valid')
